(() => {
  ...
  async function decompressString(code) {
    ...
    if (mode === 'g') {
      if (!('DecompressionStream' in window)) throw new Error('This browser cannot unpack the pairing code. Try a newer browser.');
  ...
  async function decodePayload(codeOrUrl) {
    ...
    try {
      ...
      code = new URLSearchParams(url.hash.slice(1)).get('p') || code;
    } catch (_) {
      if (code.startsWith('#')) code = new URLSearchParams(code.slice(1)).get('p') || code;
    }
    const payload = JSON.parse(await decompressString(code));
    if (payload.v !== 1 || !payload.kind || !payload.desc) throw new Error('This is not a valid ESL Walkie-Talkie pairing code.');
    return { payload, code };
  }
  ...
  function renderQR(text) {
    ...
    try {
      ...
      els.qrHint.textContent = 'This connection code is too large for a QR on this browser. Use the manual pairing code below.';
      return false;
    }
  }
  ...
  async function makeOffer() {
    ...
    els.qrHint.textContent = 'Your partner can scan this in the app or with their normal camera.';
    ...
    try {
      ...
      renderQR(`${basePageUrl()}#p=${currentPairCode}`);
  ...
  async function acceptOffer() {
    ...
    els.qrHint.textContent = `${partnerName} should scan this using the original walkie-talkie tab.`;
  ...
  function openPasteModal(purpose) {
    ...
    els.modalTitle.textContent = purpose === 'answer' ? 'Enter answer code' : 'Enter pairing code';
  ...
  async function openScanner(purpose) {
    ...
    if (!('BarcodeDetector' in window)) {
      els.scannerPanel.classList.add('hidden');
      els.pastePanel.classList.remove('hidden');
      showModalError('This browser does not provide built-in QR scanning. Use the manual code instead, or scan the first invitation with your phone\'s normal camera.');
      return;
    }
    ...
    try {
      ...
      els.scannerPanel.classList.add('hidden');
      els.pastePanel.classList.remove('hidden');
      showModalError(`${err.message} Use the manual pairing code instead.`);
    }
  }
  ...
  async function handleHashPairing() {
    ...
    try {
      ...
      if (payload.kind === 'offer') {
        ...
        message.textContent = 'This is an answer QR. Scan it from the creator device\'s open walkie-talkie page so the original connection is not lost.';
